...
public class HotelController {
    ...
    public ResponseEntity<ApiResponse<List<HotelResponse>>> getAllHotels() {
        ...
        return ResponseEntity.ok(
                new ApiResponse<>(200, "Đã lấy thành công tất cả các khách sạn", hotels)
        );
    }
    ...
    public ResponseEntity<ApiResponse<HotelResponse>> getHotelById(@PathVariable String id) {
        ...
        return ResponseEntity.ok(
                new ApiResponse<>(200, "Đã lấy thông tin chi tiết về khách sạn thành công", hotel)
        );
    }

    @Operation(summary = "Tạo mới khách sạn", description = "Thêm một khách sạn mới vào hệ thống")
    @PostMapping(UrlMapping.ADD_HOTEL)
    public ResponseEntity<ApiResponse<HotelResponse>> createHotel(@RequestBody HotelRequest request) {
        HotelResponse newHotel = hotelService.createHotel(request);
        return ResponseEntity.ok(
                new ApiResponse<>(201, "Khách sạn được tạo thành công", newHotel)
        );
    }

    @Operation(summary = "Tạo khách sạn bằng hình ảnh", description = "Thêm khách sạn mới với hình ảnh hồ sơ")
    @PostMapping(
            value = UrlMapping.ADD_HOTEL_WITH_IMAGE,
            consumes = MediaType.MULTIPART_FORM_DATA_VALUE
    )
    public ResponseEntity<ApiResponse<HotelResponse>> createHotelWithImage(
            @RequestPart("request") HotelRequest request,
            @RequestPart("image") MultipartFile image) {
        HotelResponse newHotel = hotelService.createHotelWithImage(request, image);
        return ResponseEntity.ok(
                new ApiResponse<>(201, "Khách sạn đã được tạo thành công với hình ảnh", newHotel)
        );
    }

    @Operation(summary = "Cập nhật hình ảnh khách sạn", description = "Cập nhật hình ảnh của khách sạn")
    @PostMapping(
            value = UrlMapping.UPDATE_HOTEL_IMAGE,
            consumes = MediaType.MULTIPART_FORM_DATA_VALUE
    )
    ...
    public ResponseEntity<ApiResponse<HotelResponse>> updateHotel(
        ...
        return ResponseEntity.ok(
                new ApiResponse<>(200, "Cập nhật khách sạn thành công", updatedHotel)
        );
    }
    ...
    public ResponseEntity<ApiResponse<Void>> deleteHotel(@PathVariable String id) {
        ...
        return ResponseEntity.ok(
                new ApiResponse<>(204, "Khách sạn đã được xóa thành công", null)
        );
    }
}